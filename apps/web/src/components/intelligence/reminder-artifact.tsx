...
export function ReminderArtifact({ artifact }: { artifact: ReminderArtifactData }) {
  ...
  const blocked = artifact.status.startsWith("BLOCKED") || artifact.status === "NON_SENDABLE_POLICY";
  return (
    <section className="mt-4 rounded-xl border border-sky-300/20 bg-[#071426]/75 p-4" aria-label="Prepared reminder artifact">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-sky-300">{available ? "Prepared reminder" : "Reminder eligibility"}</p><h4 className="mt-1 text-base font-semibold text-white">{artifact.customer_name}</h4><p className="mt-1 text-[10px] text-slate-500">Evaluated {new Date(artifact.prepared_at).toLocaleString()} / {artifact.intended_channel}</p></div><StatusPill tone={available ? "sky" : blocked ? "rose" : "amber"}>{artifact.status.replaceAll("_", " ")}</StatusPill></div>
      ...
      <p className={`mt-4 rounded-lg border px-3 py-2 text-xs font-semibold ${available ? "border-emerald-300/15 bg-emerald-300/[.045] text-emerald-100" : "border-amber-300/15 bg-amber-300/[.04] text-amber-100"}`}>{available ? "Prepared only — no customer communication has been sent." : "Non-sendable — no usable reminder draft was prepared or sent."}</p>
