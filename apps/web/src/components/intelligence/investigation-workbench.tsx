...
function ComparisonPanel({ result }: { result: CommandResult }) {
  ...
  return <Panel className="overflow-hidden"><SectionHeader eyebrow="Why one result ranks above another" title={"Compare " + left.entity_name + " with " + right.entity_name} detail="A factual comparison using only records returned by this command." prominent /><div className="grid gap-3 border-b border-white/[.06] p-4 sm:grid-cols-2"><CompareSelect label="First returned result" value={left.entity_id} entities={entities} onChange={setLeftId} /><CompareSelect label="Compare with" value={right.entity_id} entities={entities} onChange={setRightId} /></div><div className="grid gap-px bg-white/[.06] lg:grid-cols-2"><ComparisonSide entity={left} /><ComparisonSide entity={right} /></div><div className="space-y-3 border-t border-white/[.06] px-5 py-4 text-[15px] leading-6 text-slate-300"><p><span className="font-semibold text-sky-200">Ranking:</span> {explanation.ranking}</p><p><span className="font-semibold text-emerald-200">Operational actionability:</span> {explanation.actionability}</p>{result.query_evidence.ranking_policy.length > 0 && <p className="text-[13px] text-slate-500">Comparator: {result.query_evidence.ranking_policy.join(" → ")}.</p>}</div></Panel>;
}
