...
export type CaseApi = { case_id: string; customer_id: string; customer_name: string; evaluation: { derived_state: string; invoice: { outstanding_amount: string; days_overdue: number } | null; promises: { state: string }[]; active_dispute: boolean; eligibility: { allowed: boolean; blocking_reasons: string[] }; communication_eligibility: { consent_status: string; permitted: boolean; channel: string | null; channel_available: boolean; blocking_reasons: string[] }; next_factual_condition: string } };
...
export type RecoveryExperimentPayment = { event_id: string; day: number; event_type: string; amount: string; outstanding_before: string; outstanding_after: string; association: "NATURAL_SIMULATED_RECOVERY" | "PROMISE_FULFILMENT" | "INTERVENTION_ASSOCIATED" | "STANDARD_COLLECTIONS_ASSOCIATED"; action_reference: string | null };
export type RecoveryExperimentAction = { action_reference: string; day: number; action: string; completed: boolean; simulated_response_increment: string };
export type RecoveryExperimentArm = { account_count: number; starting_overdue_exposure: string; recovered_amount: string; recovery_rate: string; remaining_overdue: string; fully_recovered_accounts: number; median_days_to_full_recovery: string | null; average_days_to_full_recovery: string | null; actions_attempted: number; actions_completed: number; accounts_intentionally_deferred: number; dispute_contact_violations: number; active_promise_contact_violations: number; unrecovered_exceptions: number; intervention_associated_amount: string; equation_holds: boolean };
export type RecoveryExperiment = {
  experiment_version: string; seed: number; horizon_days: number;
  methodology: { design: string; assignment: string; baseline_policy: string; reconmate_policy: string; outcome_model: string; attribution_window_days: number };
  assumptions: string[];
  cohort_construction: { pair_count: number; baseline_account_count: number; reconmate_account_count: number; exact_starting_exposure_match: boolean; baseline_starting_exposure: string; reconmate_starting_exposure: string; matching_fields: string[]; exception_memberships: number; unique_accounts_with_exceptions: number; exception_categories_overlap: boolean };
  baseline: RecoveryExperimentArm; reconmate: RecoveryExperimentArm;
  difference: { recovered_amount: string; recovery_rate_percentage_points: string; remaining_overdue: string };
  claim_boundaries: { observed_portfolio_recovery: string; intervention_associated_outcomes: string; simulated_experimental_lift: string; real_world_causal_claim: string };
  evidence: { pair_id: string; customer_id: string; customer_name: string; invoice_id: string; invoice_number: string; starting_overdue_exposure: string; days_overdue_at_start: number; initial_recommendation: string; initial_blockers: string[]; exception_categories: string[]; baseline: { recovered_amount: string; remaining_overdue: string; actions: RecoveryExperimentAction[]; payments: RecoveryExperimentPayment[] }; reconmate: { recovered_amount: string; remaining_overdue: string; intentionally_deferred: boolean; decisions: string[]; actions: RecoveryExperimentAction[]; payments: RecoveryExperimentPayment[] } }[];
};
export type BatchRecoveryProof = {
  ...
  recovery_experiment: RecoveryExperiment;
};
export type ProviderEventEvidence = { id: string; payment_request_id: string; provider_event_id: string; provider_payment_reference: string; event_type: string; evidence: { source?: string; provider?: string; provider_mode?: string; provider_reference?: string; provider_payment_reference?: string; provider_status_before?: string; provider_status_after?: string; verification_state?: string; signature_verification?: string; idempotency_key?: string; immutable_event_record?: boolean; received_at?: string; validation_checks?: string[]; chronology?: string; financial_mutation?: string; outstanding_before?: string; outstanding_after?: string; score_before?: number; score_after?: number; recommendation_before?: string; recommendation_after?: string; duplicate_replay?: { ignored: boolean; original_event: string; financial_mutation: string; outstanding_before?: string; outstanding_after?: string } }; received_at: string };
export type EvidenceTimelineEntry = {
  id: string; occurred_at: string; category: "AI_INTERPRETATION" | "FACT_EVENT" | "INTELLIGENCE_REASSESSMENT" | "OPERATOR_ACTION" | "PROVIDER_EVENT" | "FINANCIAL_STATE_CHANGE";
...
export type Workspace = {
  ...
  recommendation: { recommended_action: string; priority: string; factual_reasons: string[]; blockers: string[]; relevant_days_overdue: number; human_approval_required: boolean; operator_explanation: string; operator_next_step: string; workflow_effect: string; action_readiness: { financially_actionable: boolean; no_active_blocker: boolean; communication_required: boolean; communication_permitted: boolean; consent_status: string; channel: string | null; channel_available: boolean; current_decision_valid: boolean; operator_approval: string; external_execution: string; reasons: string[] } | null; communication_signals: { intent: string; confidence: string | null }[] };
