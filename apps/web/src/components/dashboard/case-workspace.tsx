...
export function CaseWorkspace({ item, onClose, liveVersion, affected, transition }: { item: PriorityCase; onClose: () => void; liveVersion: number; affected: boolean; transition?: IntelligenceTransition }) {
  ...
  const action = useMutation({
    ...
    onError: async () => { await queryClient.invalidateQueries({ queryKey: queryKeys.workspace(item.id) }); },
  });
  ...
  return <section className="rounded-2xl border border-fuchsia-300/15 bg-fuchsia-300/[.025] p-5">
    ...
    {messages.length ? <div className="mt-4 space-y-3">{messages.map((message) => {
      ...
      return <article key={message.id} className="interactive-card rounded-xl border border-white/[.08] bg-black/10 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-slate-500">Source communication · inbound {label(message.channel)}</p><p className="mt-2 text-sm leading-6 text-slate-200">“{message.content}”</p><p className="mt-1 text-[10px] text-slate-600">{labeledTimestamp(message.occurred_at, "received")}</p></div>{!analysis && <button type="button" aria-busy={interpreting} disabled={busy || interpreting} onClick={() => void analyze(message.id)} className={buttonStyles.primary}>{interpreting ? "Extracting candidate facts…" : message.ai_processing_status === "FAILED" ? "Retry interpretation" : "Extract candidate facts"}</button>}</div>
        ...
        {!analysis && message.ai_processing_status === "FAILED" && <div className="mt-3 rounded-xl border border-rose-300/18 bg-rose-300/[.04] p-3"><div className="flex flex-wrap items-center gap-2"><StatusPill tone="rose">Failed closed</StatusPill><p className="text-xs font-semibold text-rose-100">Interpretation unavailable</p></div><p className="mt-2 text-xs leading-5 text-slate-300">{message.interpretation_failure ?? "Interpretation failed; no candidate fact was accepted."}</p><p className="mt-1 text-[11px] font-medium text-rose-100/75">No financial fact was accepted automatically. Deterministic policy remained on the previously persisted facts.</p></div>}
        {analysis && <div className="mt-4 border-t border-white/[.07] pt-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-[10px] font-bold uppercase tracking-[.12em] text-fuchsia-200">Communication interpretation</p><p className="mt-1 text-xs font-semibold text-white">{analysis.runtime_mode === "LIVE MODEL" ? "Model-backed extraction" : "Deterministic sandbox extraction"}</p></div><div className="flex flex-wrap gap-2"><StatusPill tone={analysis.runtime_mode === "LIVE MODEL" ? "emerald" : "slate"}>{analysis.runtime_mode === "LIVE MODEL" ? "Live model" : "Sandbox"}</StatusPill><StatusPill tone={analysis.review_status === "PENDING_REVIEW" ? "amber" : "slate"}>{analysis.review_status === "PENDING_REVIEW" ? "Operator review pending" : "Review recorded"}</StatusPill></div></div><p className="mt-1 text-[10px] text-slate-500">{labeledTimestamp(analysis.analyzed_at, "extracted")} · interpreted intent {label(analysis.result.intent)}</p><details className="mt-1 text-[10px] text-slate-600"><summary className="cursor-pointer select-none hover:text-slate-400">Provider and review details</summary><p className="mt-1">Provider {analysis.provider} · model {analysis.model_version ?? "not reported"} · runtime {analysis.runtime_mode}</p>{analysis.result.review_reasons?.map((reason) => <p key={reason} className="mt-1">Review reason: {reason}</p>)}</details><div className="mt-3 space-y-2">{analysis.candidates.map((candidate) => {
          ...
          const state = decision?.decision === "ACCEPT" ? "Accepted by operator" : decision?.decision === "REJECT" ? "Rejected by operator" : candidate.persistence_eligible ? "Awaiting operator confirmation" : "Needs review";
          return <div key={candidate.candidate_id} className="interactive-card rounded-xl border border-white/[.07] bg-white/[.025] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-semibold text-white">{label(candidate.fact_type)}</p><p className="mt-1 text-[10px] text-slate-500">Candidate evidence · not authoritative until accepted</p></div><div className="flex flex-wrap items-center gap-2"><StatusPill tone={decision?.decision === "ACCEPT" ? "emerald" : decision?.decision === "REJECT" ? "rose" : candidate.persistence_eligible ? "amber" : "slate"}>{state}</StatusPill><span className="text-xs font-semibold tabular-nums text-fuchsia-100">{Math.round(candidate.confidence * 100)}% confidence</span></div></div><div className="mt-3 grid gap-3 lg:grid-cols-2"><div><p className="text-[10px] font-bold uppercase tracking-[.11em] text-slate-500">Extracted candidate value</p><CandidateValue data={candidate.proposed_data} /></div><div><p className="text-[10px] font-bold uppercase tracking-[.11em] text-slate-500">Exact supporting span</p><blockquote className="mt-1.5 border-l-2 border-fuchsia-300/35 pl-3 text-xs leading-5 text-slate-300">“{candidate.evidence_span}”</blockquote></div></div><p className={`mt-3 text-xs ${candidate.persistence_eligible ? "text-emerald-200" : "text-amber-200"}`}><strong>Server validation:</strong> {candidate.persistence_eligible ? "Source-grounded and eligible for operator confirmation." : candidate.defer_reason ?? "Not eligible for persistence."}</p><p className="mt-1 text-[11px] text-slate-500">Human control: {candidate.operator_confirmation_required ? "explicit operator confirmation required" : "no operator confirmation requirement reported"}.</p>{decision ? decision.decision === "ACCEPT" ? <AcceptedFactImpact decision={decision} /> : <div className="mt-3 rounded-xl border border-rose-300/12 bg-rose-300/[.025] p-3"><div className="flex flex-wrap items-center gap-2"><StatusPill tone="rose">Candidate rejected</StatusPill><span className="text-[11px] text-slate-400">Operator {decision.operator_id}</span></div><p className="mt-2 text-xs text-slate-400">No candidate fact was persisted, deterministic policy stayed on prior facts, and no financial state changed.</p></div> : <div className="mt-3 flex flex-wrap gap-2"><button type="button" aria-busy={accepting} disabled={busy || !candidate.persistence_eligible || !workspace.invoice} onClick={() => void decide(message.id, analysis.id, candidate.candidate_id, "ACCEPT")} className={buttonStyles.success}>{accepting ? "Accepting fact…" : "Accept structured fact"}</button><button type="button" aria-busy={rejecting} disabled={busy} onClick={() => void decide(message.id, analysis.id, candidate.candidate_id, "REJECT")} className={buttonStyles.secondary}>{rejecting ? "Rejecting candidate…" : "Reject candidate"}</button>{!candidate.persistence_eligible && <p className="w-full text-[11px] text-amber-100/70">No financial fact was accepted automatically. This candidate cannot be persisted from the current interpretation.</p>}</div>}</div>;
        })}{!analysis.candidates.length && <div className="rounded-xl border border-amber-300/15 bg-amber-300/[.035] p-3"><StatusPill tone="amber">No candidate accepted</StatusPill><p className="mt-2 text-xs leading-5 text-slate-300">The interpretation produced no supported candidate evidence. No financial fact was accepted automatically.</p></div>}</div></div>}
      </article>;
...
function CandidateValue({ data }: { data: Record<string, string | boolean | null> }) {
  const entries = Object.entries(data).filter(([, value]) => value !== null && value !== "");
  if (!entries.length) return <p className="mt-1.5 text-xs text-slate-500">No structured value proposed.</p>;
  return <dl className="mt-1.5 space-y-1 text-xs">{entries.map(([key, value]) => <div key={key} className="flex flex-wrap gap-2"><dt className="text-slate-500">{label(key)}:</dt><dd className="font-semibold text-slate-200">{typeof value === "boolean" ? value ? "Yes" : "No" : value}</dd></div>)}</dl>;
}
...
function AcceptedFactImpact({ decision }: { decision: NonNullable<Workspace["communications"][number]["analyses"][number]["candidates"][number]["decision_result"]> }) {
  ...
  const policyChanged = decision.score_before !== decision.score_after || blockersBefore !== blockersAfter || decision.recommendation_before !== decision.recommendation_after;
  return <div className="live-enter mt-3 rounded-xl border border-emerald-300/25 bg-emerald-300/[.055] p-4 shadow-[0_12px_30px_rgba(52,211,153,.06)]">
    <div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-[10px] font-extrabold uppercase tracking-[.14em] text-emerald-300">Accepted evidence → deterministic policy effect</p><p className="mt-1 text-sm font-semibold text-white">Persisted fact: {decision.persisted_fact ?? "Confirmed structured fact"}</p></div><StatusPill tone={policyChanged ? "emerald" : "slate"}>{policyChanged ? "Policy changed" : "Decision held"}</StatusPill></div>
    ...
    <p className="mt-3 text-[11px] leading-5 text-emerald-100/70">{policyChanged ? "The accepted evidence materially changed at least one deterministic policy output shown above." : "Deterministic policy was reassessed, but the score, blockers, and recommendation did not materially change."} No invoice amount, payment, promise amount, or balance was changed.</p>
  </div>;
}
  ...
  return <section id="case-evidence-timeline" className="scroll-mt-6 rounded-2xl border border-white/[.09] bg-white/[.025] p-5">
    ...
    {entries.length ? <ol className="mt-5 space-y-3">{entries.map((entry) => {
      ...
      return <li key={entry.id} className={`interactive-card rounded-r-xl border-l-2 bg-black/[.08] py-2.5 pl-4 pr-3 ${tones[entry.category]}`}>
        ...
        {entry.detail && <p className="mt-1 text-xs leading-5 text-slate-400">{entry.category === "AI_INTERPRETATION" ? entry.detail : label(entry.detail)}</p>}
