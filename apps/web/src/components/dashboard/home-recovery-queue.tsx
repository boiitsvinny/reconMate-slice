    ...
    <Panel className="mt-5 overflow-hidden">
      ...
      <div className="divide-y divide-white/[.055]">
        {visible.map((item) => {
          ...
          return <button type="button" key={item.customerId} onClick={() => onSelect(item)} className="grid w-full gap-3 px-4 py-4 text-left transition hover:bg-white/[.025] lg:grid-cols-[minmax(180px,1.1fr)_110px_minmax(190px,1.2fr)_minmax(190px,1.15fr)_110px] lg:items-center lg:gap-4 lg:px-5"><div><p className="truncate text-sm font-semibold text-white">{item.customerName}</p><p className="mt-1 text-[10px] text-slate-600">{event ? `Reassessed ${new Date(event.occurred_at).toLocaleDateString()}` : `Evaluated ${intel?.calculated_at ?? "currently"}`}</p></div><div><p className="text-sm font-semibold tabular-nums text-slate-200">{item.amount}</p><p className="mt-1 text-[10px] text-slate-500">{item.daysOverdue}d overdue</p></div><div><p className="text-xs font-semibold text-sky-200">{transition?.previous_recommendation ? `${transition.previous_recommendation.replaceAll("_", " ")} → ${transition.current_recommendation_title}` : intel?.recommendation.title ?? item.recommendedAction.replaceAll("_", " ")}</p>{intel && <p className="mt-1 text-[10px] tabular-nums text-slate-500">Score {transition?.previous_score !== null && transition?.previous_score !== undefined ? `${transition.previous_score} → ` : ""}{intel.score}/100</p>}</div><p className="line-clamp-2 text-xs leading-5 text-slate-400">{trigger}</p><StatusPill tone={status === "Approval" ? "amber" : status === "Blocked" ? "rose" : status === "Monitoring" ? "emerald" : "sky"}>{status}</StatusPill></button>;
        })}
