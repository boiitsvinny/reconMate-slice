...
export function PortfolioMetricCard({ label, value, detail, impact, tone = "blue" }: { label: string; value: string; detail: string; impact?: string; tone?: Tone }) {
  ...
  return <article className={`group relative min-h-[178px] overflow-hidden border p-6 transition duration-300 hover:-translate-y-0.5 ${tones[tone]}`}>
    <div className="absolute inset-x-0 top-0 h-px bg-current opacity-30" />
    <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">{label}</p>
    <p className="mt-5 text-3xl font-semibold tracking-[-0.05em] text-white sm:text-[38px]">{value}</p>
    <p className="mt-4 text-xs text-slate-500">{detail}</p>
    {impact && <p className="mt-2 text-[11px] font-medium text-sky-300">{impact}</p>}
  </article>;
}
