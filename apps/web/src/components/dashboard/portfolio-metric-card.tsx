...
export function PortfolioMetricCard({ label, value, detail, impact, state, tone = "blue", className }: { label: string; value: string; detail: string; impact?: string; state?: string; tone?: Tone; className?: string }) {
  ...
  return (
    <article className={cx("group relative min-h-[126px] overflow-hidden rounded-2xl border bg-[#08111f]/95 p-4 shadow-[0_14px_36px_rgba(0,0,0,.16)] transition duration-300 hover:-translate-y-0.5", tones[tone], className)}>
      ...
      <p className="mt-3 text-2xl font-semibold tracking-[-0.035em] text-white">{value}</p>
