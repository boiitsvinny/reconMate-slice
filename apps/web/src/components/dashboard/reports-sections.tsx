...
function ExecutiveSnapshot({ recovery, intelligence, latestCycle, events, actions }: ReportProps) {
  ...
  return <Panel className="overflow-hidden"><SectionHeader eyebrow="Portfolio position" title="Executive Recovery Snapshot" detail={`Current persisted position evaluated across ${intelligence.customer_count} customer accounts.`} prominent /><div className="grid grid-cols-2 gap-px bg-white/[.06] md:grid-cols-3 xl:grid-cols-6"><ReportMetric label="Overdue exposure" value={formatMoney(recovery.overdue_exposure)} detail="Outstanding receivables past due" tone="rose" /><ReportMetric label="Elevated accounts" value={String(elevated)} detail={`${intelligence.level_counts.CRITICAL} critical · ${intelligence.level_counts.HIGH} high`} tone="amber" /><ReportMetric label="Payments this cycle" value={recovered ? formatMoney(recovered) : "—"} detail="Sum of persisted payment events" tone="emerald" /><ReportMetric label="Current blockers" value={String(recovery.cases_blocked_by_dispute + recovery.cases_awaiting_payment)} detail={`${recovery.cases_blocked_by_dispute} disputes · ${recovery.cases_awaiting_payment} promises`} tone="sky" /><ReportMetric label="Awaiting approval" value={String(awaiting)} detail="Workflow actions pending approval" tone="amber" /><ReportMetric label="Executed workflows" value={String(executed)} detail="Internal workflow records executed" tone="emerald" /></div></Panel>;
}
...
function OperatorActivity({ actions, paymentRequests, casesById, onSelectCase }: ReportProps & { casesById: Map<string, PriorityCase> }) {
  ...
  return <Panel className="overflow-hidden">
    ...
    {visibleRequests.length > 0 && <div className="divide-y divide-white/[.055] border-b border-white/[.06]">{visibleRequests.map((request) => {
      ...
      return <article key={request.id} className="interactive-row grid gap-3 border-l-[3px] border-l-violet-300/45 p-5 lg:grid-cols-[minmax(190px,.8fr)_minmax(260px,1fr)_140px_minmax(190px,.8fr)_auto] lg:items-center"><div><p className="text-base font-semibold text-white">{recoveryCase?.customerName ?? `Case ${request.case_id.slice(0, 8)}`}</p><p className="mt-1 text-xs text-violet-200">{request.provider_mode === "TEST" ? "Razorpay test mode" : "Provider Demo Mode"}</p></div><div><p className="text-[13px] font-semibold text-sky-200">Payment request · {formatMoney(request.requested_amount)}</p><p className="mt-1 text-xs text-slate-400">Reference {request.provider_reference ?? "not issued"}{Number(request.paid_amount) ? ` · ${formatMoney(request.paid_amount)} payment recorded` : " · no payment fact yet"}</p></div><StatusPill tone={request.status === "FAILED" ? "rose" : request.status === "PAID" ? "emerald" : "sky"}>{label(request.status)}</StatusPill><div><p className="text-xs text-slate-300">{request.operator_id}</p><p className="mt-1 text-[11px] text-slate-500">{request.created_at ? new Date(request.created_at).toLocaleString() : "Timestamp unavailable"}</p></div>{recoveryCase && <button type="button" onClick={() => onSelectCase(recoveryCase)} className={buttonStyles.secondary}>Open evidence</button>}</article>;
    })}</div>}
