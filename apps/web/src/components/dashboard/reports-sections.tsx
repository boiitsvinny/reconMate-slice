...
function BatchRecoveryProofPanel({ batchRecovery: proof, casesById, onSelectCase }: ReportProps & { casesById: Map<string, PriorityCase> }) {
  ...
  return <Panel className="overflow-hidden border-sky-300/20">
    ...
    <div className="grid gap-px bg-white/[.06] lg:grid-cols-[1fr_auto_1fr_auto_1fr_.8fr] lg:items-stretch">
      ...
      <span className="hidden place-items-center bg-[#08111f] px-1 text-xl text-sky-300 lg:grid">=</span>
      <ProofAmount label="Current overdue exposure" value={proof.reconciliation.remaining_overdue_exposure} detail={`${proof.reconciliation.remaining_open_overdue_invoice_count} invoice(s) remain open`} tone="rose" meta={proof.metric_metadata.remaining_overdue_exposure} />
...
function ProofAmount({ label: amountLabel, value, detail, tone = "sky", money = true, meta }: { label: string; value: string; detail: string; tone?: "sky" | "emerald" | "rose"; money?: boolean; meta?: { unit: string; scope: string; window: string } }) {
  ...
  return <article className="bg-[#08111f] p-5 sm:p-6"><p className="text-[10px] font-bold uppercase tracking-[.13em] text-slate-400">{amountLabel}</p><p className={`mt-2 text-2xl font-semibold tabular-nums tracking-[-.03em] ${colors[tone]}`}>{money ? formatMoney(value) : value}</p><p className="mt-1 text-[11px] leading-5 text-slate-500">{detail}</p>{meta && <p className="mt-2 text-[9px] uppercase leading-4 tracking-[.09em] text-slate-600">Unit {meta.unit} · scope {meta.scope} · {meta.window}</p>}</article>;
}
